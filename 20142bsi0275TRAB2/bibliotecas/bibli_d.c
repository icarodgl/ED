...
void excluir_node(list *lista, node *nk){
...
void inversor(list *lista){
	...
	for(rastro=lista->inicio; rastro != NULL; rastro= rastro->proximo){
			rastro->anterior=rastro->proximo;
		}
		for(rastro=lista->inicio; rastro != NULL; rastro= rastro->anterior){
			rastro->proximo=lastro;
			lastro=rastro;
		}
		rastro= lista->inicio;
		lastro= lista->fim;
...
node *caminha(list *lista, int pos){
	int i;
	node *ra=NULL;
	if(pos >= lista->tamanho){return ra;}
	ra=lista->inicio;
	for (i=0; i < pos; i++){
	ra=ra->proximo;
	}
